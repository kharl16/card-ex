...
/**
 * Lets a card owner toggle which global package photos appear on their card.
 * Mirrors GlobalProductsManager but writes to card_global_package_overrides.
 */
export default function GlobalPackagesManager({ cardId }: Props) {
  const { allGlobals, hiddenIds, loading, setHiddenLocal } = useGlobalPackageImages(cardId);
  ...
  async function toggle(globalImageId: string) {
    ...
    const { error } = wasHidden
      ? await supabase
          ...
          .delete()
          .eq("card_id", cardId)
          .eq("global_package_image_id", globalImageId)
      : await supabase
          .from("card_global_package_overrides")
          .insert({ card_id: cardId, global_package_image_id: globalImageId });
  ...
  if (loading) return <div className="text-sm text-muted-foreground">Loading shared packages…</div>;
  if (allGlobals.length === 0) return null;

  return (
    <div className="space-y-3">
      <div>
        <h3 className="text-lg font-semibold">Shared Package Photos</h3>
        <p className="text-sm text-muted-foreground">
          These packages appear on every card by default. Hide any you don't want on your card.
        </p>
      </div>
