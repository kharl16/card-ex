...
/**
 * Lets a card owner toggle which global testimony photos appear on their card.
 * Mirrors GlobalPackagesManager but writes to card_global_testimony_overrides.
 */
export default function GlobalTestimoniesManager({ cardId }: Props) {
  const { allGlobals, hiddenIds, loading, setHiddenLocal } = useGlobalTestimonyImages(cardId);
  ...
  async function toggle(globalImageId: string) {
    ...
    window.dispatchEvent(
      new CustomEvent("global-testimony-override-changed", {
    ...
    const { error } = wasHidden
      ? await supabase
          ...
          .delete()
          .eq("card_id", cardId)
          .eq("global_testimony_image_id", globalImageId)
      : await supabase
          .from("card_global_testimony_overrides")
          .insert({ card_id: cardId, global_testimony_image_id: globalImageId });
    ...
    if (error) {
      ...
      toast.success(wasHidden ? "Testimony will show on your card" : "Testimony hidden from your card");
    }
    setBusyId(null);
  }

  if (loading) return <div className="text-sm text-muted-foreground">Loading shared testimonies…</div>;
  if (allGlobals.length === 0) return null;

  return (
    <div className="space-y-3">
      <div>
        <h3 className="text-lg font-semibold">Shared Testimony Photos</h3>
        <p className="text-sm text-muted-foreground">
          These testimonies appear on every card by default. Hide any you don't want on your card.
        </p>
      </div>
