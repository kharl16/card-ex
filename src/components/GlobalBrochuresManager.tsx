...
/**
 * Lets a card owner toggle which global brochure photos appear on their card.
 * Mirrors GlobalPackagesManager but writes to card_global_brochure_overrides.
 */
export default function GlobalBrochuresManager({ cardId }: Props) {
  const { allGlobals, hiddenIds, loading, setHiddenLocal } = useGlobalBrochureImages(cardId);
  ...
  async function toggle(globalImageId: string) {
    ...
    window.dispatchEvent(
      new CustomEvent("global-brochure-override-changed", {
    ...
    const { error } = wasHidden
      ? await supabase
          ...
          .delete()
          .eq("card_id", cardId)
          .eq("global_brochure_image_id", globalImageId)
      : await supabase
          .from("card_global_brochure_overrides")
          .insert({ card_id: cardId, global_brochure_image_id: globalImageId });
    ...
    if (error) {
      ...
      toast.success(
        wasHidden ? "Brochure page will show on your card" : "Brochure page hidden from your card"
      );
    }
    setBusyId(null);
  }

  if (loading) return <div className="text-sm text-muted-foreground">Loading shared brochure…</div>;
  if (allGlobals.length === 0) return null;

  return (
    <div className="space-y-3">
      <div>
        <h3 className="text-lg font-semibold">Shared Brochure Photos</h3>
        <p className="text-sm text-muted-foreground">
          These brochure pages appear on every card by default. Hide any you don't want on your card.
        </p>
      </div>
