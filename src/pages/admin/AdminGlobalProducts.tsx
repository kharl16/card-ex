...
export default function AdminGlobalProducts() {
  ...
  async function move(row: Row, dir: -1 | 1) {
    ...
    const results = await Promise.all(
      changed.map(({ r, i }) =>
        supabase.from("global_product_images").update({ sort_index: i }).eq("id", r.id)
      )
    );
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-8 rounded-xl border border-border bg-card p-4 space-y-4">
        <AdminPhotoUploader busy={busy} onFilesSelected={onFileChoose} label="Upload product photos" />
