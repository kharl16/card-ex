...
export default function AdminGlobalPackages() {
  ...
  async function addRow(url: string, caption: string) {
    ...
    const { error } = await supabase.from("global_package_images").insert({
      url,
  ...
  async function remove(row: Row) {
    ...
    const { error } = await supabase.from("global_package_images").delete().eq("id", row.id);
    if (error) toast.error(error.message);
    else {
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Global Package Photos</h1>
      ...
      <div className="mb-8 rounded-xl border border-border bg-card p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            />
            <Input
              placeholder="Caption (e.g. Hope, Copper, Bronze)"
              value={captionInput}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
          No global packages yet. Upload one above and it will appear on every card.
        </div>
      ) : (
