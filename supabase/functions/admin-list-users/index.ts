...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Prefer Supabase-auth user lookup, but gracefully handle "session_not_found" cases.
    let currentUserId: string | null = null;
    const { data: userData, error: userError } = await userClient.auth.getUser();
    if (userData?.user?.id) {
      currentUserId = userData.user.id;
    } else {
      console.warn("admin-list-users: auth.getUser failed, falling back to JWT sub", {
